...
export default function Footer() {
  return (
    <footer style={{ background:"linear-gradient(165deg, #091E10 0%, #173D26 55%, #091408 100%)", color:"white", position:"relative", overflow:"hidden" }}>
      {/* Top accent */}
      <div style={{ height:3, background:"linear-gradient(90deg, var(--c-forest), var(--c-amber), var(--c-leaf), var(--c-amber), var(--c-forest))" }} />

      {/* BG glows */}
      <div style={{ position:"absolute", top:"-10%", right:"-5%", width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle, rgba(77,170,124,0.10), transparent)", filter:"blur(70px)", pointerEvents:"none" }} />
      <div style={{ position:"absolute", bottom:"-10%", left:"-5%", width:350, height:350, borderRadius:"50%", background:"radial-gradient(circle, rgba(200,128,26,0.08), transparent)", filter:"blur(70px)", pointerEvents:"none" }} />
      ...
      <div className="section-container" style={{ position:"relative", zIndex:1, paddingTop:64, paddingBottom:0 }}>
        <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr 1.6fr", gap:48, paddingBottom:48, borderBottom:"1px solid rgba(255,255,255,0.08)" }} className="footer-grid">
          ...
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:20 }}>
              <div style={{ width:52, height:52, borderRadius:14, overflow:"hidden", border:"2px solid rgba(200,128,26,0.35)", flexShrink:0 }}>
                <img src="/logo.jpeg" alt="United for Animal Welfare" style={{ width:"100%", height:"100%", objectFit:"cover" }} />
              </div>
              <div>
                <p style={{ fontFamily:"var(--f-body)", fontSize:"0.62rem", fontWeight:700, letterSpacing:"0.2em", color:"var(--c-amber-light)", textTransform:"uppercase", marginBottom:3 }}>United for</p>
            ...
            <p style={{ fontFamily:"var(--f-body)", fontSize:"0.8rem", color:"rgba(200,128,26,0.55)", fontStyle:"italic", marginBottom:22 }}>"Compassion For Every Life"</p>
            <div style={{ display:"flex", gap:10 }}>
              {[FaInstagram, FaLinkedinIn, FaFacebookF, FaWhatsapp].map((Icon,i) => (
                <motion.a key={i} href="#"
                  ...
                  whileHover={{ scale:1.12, y:-2, background:"rgba(200,128,26,0.20)" }} transition={{ type:"spring", stiffness:380 }}>
          ...
          {/* Quick Links */}
          <div>
            <p style={{ fontFamily:"var(--f-body)", fontSize:"0.65rem", fontWeight:700, letterSpacing:"0.18em", textTransform:"uppercase", color:"var(--c-amber)", marginBottom:20 }}>Quick Links</p>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {LINKS.map((item,i) => (
                <Link key={i} to={item.path}
                  ...
                  <ChevronRight style={{ width:12, height:12, color:"var(--c-amber-light)", flexShrink:0 }} />
                  {item.name}
                </Link>
              ))}
            </div>
          ...
          {/* Contact */}
          <div>
            <p style={{ fontFamily:"var(--f-body)", fontSize:"0.65rem", fontWeight:700, letterSpacing:"0.18em", textTransform:"uppercase", color:"var(--c-amber)", marginBottom:20 }}>Get In Touch</p>
            <div style={{ display:"flex", flexDirection:"column", gap:14, marginBottom:22 }}>
              {[{ Icon:MapPin, text:"Rajasthan, India · Registered Public Trust, 2026" },
                ...
                <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                  <div style={{ width:32, height:32, borderRadius:8, background:"rgba(200,128,26,0.12)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                    <Icon style={{ width:14, height:14, color:"var(--c-amber-light)" }} />
                  </div>
            ...
            <div style={{ background:"rgba(255,255,255,0.04)", borderRadius:14, padding:"14px 16px", border:"1px solid rgba(255,255,255,0.07)", marginBottom:22 }}>
              <p style={{ fontSize:"0.62rem", fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", color:"var(--c-amber-light)", marginBottom:10 }}>Key Contacts</p>
        ...
        <div style={{ padding:"20px 0", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
          ...
          <p style={{ fontFamily:"var(--f-body)", fontSize:"0.78rem", color:"rgba(200,128,26,0.40)", fontStyle:"italic" }}>
